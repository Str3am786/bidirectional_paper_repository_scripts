...
\usepackage[textsize=tiny, textwidth=60mm, disable]{todonotes}
% \usepackage[textsize=tiny, textwidth=60mm]{todonotes}
...
%distort the distribution of this 
...
%, and the causal relations between the variables at play, 
...
% We refer to the data used to fit and evaluate a biomarker (for example a dataset
% collected for research) as source data and the data on which the model is
% applied to obtain useful predictions (for example the visitors of a hospital) as
% target data.
%
...
% Several results from domain adaptation theory \citep{redko2019advances} bound
% the error on the target distribution for covariate shift using the error on the
% source distribution and a term reflecting the divergence between the marginal
% distributions of \(X\) in source and target data. Consistently with these
% results, some approaches to dataset shift aim at bringing the empirical
% distribution of \(X\) in the reweighted training sample close to that of an
% unlabelled sample from the target distribution, while retaining a good
% prediction performance on source data. This transformation of input features can
% be performed for example by aligning moments, minimizing a discrepancy such as
% the MMD, or with Optimal Transport.
...
% Rather than relying on an uncertainty set, \citet{subbaswamy2019preventing} propose to exploit knowledge of the causal structure of the problem to learn features that are independent of aspects that are likely to differ in the target data.
...
% \citet{krueger2020out,khani2019maximum,duchi2018learning,maurer2009empirical}.
...
% Since the seminal work of \citet{ben2010theory}, several papers have provided
% bounds for the error on target data of a predictor trained on a sample from the
% source distribution.
% %
% Most bound the error on the target distribution by a sum of (i) the error on the
% source distribution, (ii) a term reflecting the divergence between the marginal
% distributions of \(X\) in source and target data and (iii) a term reflecting how
% different are the true functions from \(X\) to \(Y\) in source and target data,
% assumed to be small -- and impossible to estimate without labelled target data.
% \Citet{redko2019advances} provide a comprehensive review as well as high-level
% insights.
% %

% Consistently with these results, some approaches to dataset shift (including
% some strategies for setting importance weights) assume that the true mappings
% from \(X\) to \(Y\) are similar in source and target data (that the shift is
% close to a covariate shift), and then aim at bringing the empirical distribution
% of \(X\) in the reweighted training sample close to that of an unlabelled sample
% from the target distribution, while retaining a good prediction performance on
% source data.
...
%
% see also fig. 1 in shimodaira2000improving, reused in several works. should we
% use that one?
\begin{figure}
\centering
...
% \todo{Jerome: actually they are different random variables; eg \(X^{(s)},
%   X^{(t)}\); for (apparent) simplicity we give them the same name, which is why
%   we distinguish \(\E_s, \E_t\) below -- it makes less sense but results in
%   less symbols on the page}
%
If the support of \(p_t\) is included in that of \(p_s\) (meaning that \(p_s > 0
\) wherever \( p_t > 0\)), we have:
% (see for example \citep{sugiyama2012machine,cortes2008sample,huang2007correcting})
%
\begin{equation}
\E_{\text{source}}[\,L(Y,\,f(X))\,] = \E_{\text{target}}\left[\,\frac{p_t(X,\,Y)}{p_s(X,\,Y)}\;L(Y,\,f(X))\,\right] \; ,
\end{equation}
%
%where \(L\) is the cost function and \(f\) is a prediction function.
...
%
%
% Large values of w indicate little overlap of source and target distributions and
% bad generalization
%
...
parameters.
%
% -- using a distribution of weights with a low entropy reduces the
% effective training sample size.
%
...
Finally, Importance Weighting (\Cref{sec:importance-weighting})\todo[inline]{Gael: The narrative is awkward, because we
are discussing importance weighting before introducing it. I wonder if we
should revisit the order of the sections? Jerome: note that would mean
moving figure 1 towards the end of the paper. I changed ``IW'' to
``reweighting'' so we only use the intuiton but not the term
``importance'' until IW is introduced} improves the predictions of the
